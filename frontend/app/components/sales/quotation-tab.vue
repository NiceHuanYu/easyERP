  ...
  <div class="tab-body">
    <div class="toolbar">
      <div class="toolbar-left">
        <div class="search-box"><span class="search-icon">🔍</span><input v-model="s" type="text" placeholder="搜索报价单号、客户..." class="search-input" /></div>
        <select v-model="fs" class="filter-select"><option value="">全部状态</option><option v-for="st in ss" :key="st" :value="st">{{ st }}</option></select>
      </div>
      <div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建报价</button></div>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="row in paged" :key="row.code">
            ...
            <td class="spec">{{ row.material }}</td><td class="num">{{ row.amount.toFixed(2) }}</td>
            <td>V{{ row.version }}</td><td class="spec">{{ row.validUntil }}</td>
    ...
    <FormModal :show="showForm" :title="editing?'编辑报价':'新建报价'" @close="showForm=false" @save="save">
      <div class="grid">
        <div class="fg"><label>报价单号</label><input v-model="f.code" :disabled="!editing" /></div>
        <div class="fg"><label>客户名称 <span class="req">*</span></label><input v-model="f.customer" placeholder="客户名称" /></div>
        <div class="fg"><label>物料名称 <span class="req">*</span></label><input v-model="f.material" placeholder="物料/产品名称" /></div>
        <div class="fg"><label>报价金额 (元)</label><input v-model.number="f.amount" type="number" step="0.01" placeholder="0.00" /></div>
        <div class="fg"><label>版本</label><input v-model="f.version" type="text" placeholder="1.0" /></div>
        <div class="fg"><label>有效期至</label><input v-model="f.validUntil" type="text" placeholder="2025-12-31" /></div>
...
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;f.code=`QTN-${String(data.value.length+1).padStart(3,'0')}`;f.customer='';f.material='';f.amount=0;f.version=1;f.validUntil='';f.status='草稿';f.remark=''}showForm.value=true}
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;background:#e8f0fe;color:#1a73e8;}.tag.draft{background:#f5f5f5;color:#999;}.tag.sent{background:#fff3e0;color:#e65100;}.tag.confirmed{background:#e8f5e9;color:#2e7d32;}.tag.expired{background:#fce4ec;color:#c62828;}

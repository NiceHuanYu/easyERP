    ...
    <div class="tracking-list">
      <div v-for="row in paged" :key="row.code" class="track-card">
        <div class="track-header">
          ...
          <span :class="['erp-tag', row.sc]">{{ row.status }}</span>
        </div>
      ...
      <div v-if="paged.length===0" class="erp-cell-empty">暂无跟踪数据</div>
    </div>
...
<style scoped>
.erp-tag{display:inline-block;padding:3px 12px;border-radius:12px;font-size:12px;font-weight:500;}

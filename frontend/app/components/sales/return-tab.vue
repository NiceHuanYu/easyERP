<template>
  <div class="tab-body">
    ...
    <FormModal :show="showForm" :title="editing?'编辑退货单':'新建退货单'" @close="showForm=false" @save="save">
      <div class="grid">
        <div class="fg"><label>退货单号</label><input v-model="f.code" :disabled="!editing" /></div>
        <div class="fg"><label>关联订单</label><input v-model="f.soCode" placeholder="SO-2025-xxxx" /></div>
...
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;f.code=`RMA-${String(data.value.length+1).padStart(3,'0')}`;f.soCode='';f.customer='';f.material='';f.qty=1;f.unit='台';f.reason='';f.status='待检验';f.remark=''}showForm.value=true}
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.pending{background:#fff3e0;color:#e65100;}.tag.received{background:#e8f5e9;color:#2e7d32;}
.tag.refunded{background:#e3f2fd;color:#1565c0;}.tag.closed{background:#f5f5f5;color:#999;}

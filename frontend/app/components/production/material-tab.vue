  ...
  <div class="tab-body">
    <div class="toolbar">
      <div class="toolbar-left">
        <div class="search-box"><span class="search-icon">🔍</span><input v-model="s" type="text" placeholder="搜索单号、工单号..." class="search-input" /></div>
        <select v-model="ft" class="filter-select"><option value="">全部类型</option><option value="领料">领料</option><option value="退料">退料</option></select>
        <select v-model="fs" class="filter-select"><option value="">全部状态</option><option v-for="st in ss" :key="st" :value="st">{{ st }}</option></select>
      </div>
      <div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建领/退料单</button></div>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="row in paged" :key="row.code">
            ...
            <td><span :class="['tag', row.type==='领料'?'pick':'return']">{{ row.type }}</span></td>
            <td class="spec">{{ row.moCode }}</td><td class="spec">{{ row.material }}</td>
            <td class="num">{{ row.qty }}{{ row.unit }}</td><td>{{ row.warehouse }}</td>
            <td>{{ row.applicant }}</td><td class="spec">{{ row.date }}</td>
    ...
    <FormModal :show="showForm" :title="editing?'编辑单据':'新建单据'" @close="showForm=false" @save="save">
      <div class="grid">
        <div class="fg"><label>单号</label><input v-model="f.code" :disabled="!editing" /></div>
        <div class="fg"><label>类型</label><select v-model="f.type"><option value="领料">领料</option><option value="退料">退料</option></select></div>
        ...
        <div class="fg"><label>数量</label><input v-model.number="f.qty" type="number" step="0.01" min="0.01" /></div>
        ...
        <div class="fg"><label>仓库</label><input v-model="f.warehouse" placeholder="如 原材料库" /></div>
        <div class="fg"><label>申请人</label><input v-model="f.applicant" placeholder="姓名" /></div>
...
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;f.code=`MTL-${String(data.value.length+1).padStart(3,'0')}`;f.type='领料';f.moCode='';f.material='';f.qty=1;f.unit='个';f.warehouse='';f.applicant='';f.date='';f.status='草稿';f.remark=''}showForm.value=true}
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.pick{background:#e3f2fd;color:#1565c0;}.tag.return{background:#fff3e0;color:#e65100;}
.tag.draft{background:#f5f5f5;color:#999;}.tag.approved{background:#e8f5e9;color:#2e7d32;}.tag.done{background:#e8f5e9;color:#2e7d32;}.tag.returned{background:#fce4ec;color:#c62828;}

<template>
  <div class="erp-tab-body">
    ...
    <div class="erp-table-wrap">
      <table class="erp-table">
        <thead><tr>
          ...
          <th>类型</th><th>关联工单</th><th>产品明细</th><th style="text-align:right;">总数量</th>
        ...
        <tbody>
          <tr v-for="row in paged" :key="row.code">
            ...
            <td class="erp-cell-spec">{{ row.moCode }}</td><td class="erp-cell-spec">{{ summary(row.items) }}</td>
            <td class="erp-cell-num">{{ totalQty(row.items) }}</td><td>{{ row.warehouse }}</td>
            ...
            <td class="erp-cell-acts">
              <button v-if="row.status==='草稿'" class="erp-lnk" style="color:#2e7d32;" @click="submit(row)">提交</button>
    ...
    <!-- 明细查看弹窗 -->
    <FormModal :show="showDetail" :title="'单据明细 - '+detailCode" @close="showDetail=false" @save="showDetail=false">
      <div style="margin-bottom:8px;font-size:13px;color:#888;">类型：{{ detailType }} ｜ 工单：{{ detailMoCode }} ｜ 仓库：{{ detailWarehouse }} ｜ 申请人：{{ detailApplicant }} ｜ 日期：{{ detailDate }}</div>
      <div class="erp-table-wrap">
        <table class="erp-table">
    ...
    <!-- 表单弹窗 -->
    <FormModal
      :show="showForm" :title="editing?'编辑单据':'新建单据'"
      size="lg"
...
function totalQty(items: MaterialItem[]) { return items.reduce((s,i) => s + (i.qty||0), 0) }
function summary(items: MaterialItem[]) {
  if (!items.length) return '-'
  return items.length === 1 ? items[0].material : `${items[0].material} 等${items.length}项`
}

const data = ref<MaterialOrder[]>([
  { code:'MTL-001',type:'领料',moCode:'MO-2025-0045',items:[{material:'45#圆钢 Φ20',qty:120,unit:'kg'}],warehouse:'原材料库',applicant:'张工',date:'2025-08-01',status:'已领用',remark:'',sc:'done'},
  { code:'MTL-002',type:'领料',moCode:'MO-2025-0045',items:[{material:'M8×30六角螺栓',qty:200,unit:'个'}],warehouse:'辅料库',applicant:'张工',date:'2025-08-01',status:'已审核',remark:'',sc:'approved'},
  { code:'MTL-003',type:'退料',moCode:'MO-2025-0042',items:[{material:'Q235槽钢 10#',qty:2,unit:'根'}],warehouse:'原材料库',applicant:'赵工',date:'2025-08-02',status:'已退回',remark:'余料退回',sc:'returned'},
  { code:'MTL-004',type:'领料',moCode:'MO-2025-0044',items:[{material:'304不锈钢板 2mm',qty:5,unit:'张'}],warehouse:'原材料库',applicant:'李工',date:'2025-08-05',status:'草稿',remark:'',sc:'draft'},
  { code:'MTL-005',type:'退料',moCode:'MO-2025-0041',items:[{material:'铜排 TMY-40×4',qty:3,unit:'m'}],warehouse:'原材料库',applicant:'王工',date:'2025-07-25',status:'已退回',remark:'',sc:'returned'},
])

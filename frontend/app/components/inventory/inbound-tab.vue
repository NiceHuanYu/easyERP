...
<div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索入库单号、物料..." class="search-input"/></div><select v-model="ft" class="filter-select"><option value="">全部类型</option><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建入库单</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">入库单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>类型</th><th>物料</th><th style="text-align:right;">数量</th><th>仓库</th><th>来源单号</th><th>日期</th><th>状态</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td><span class="tag">{{ r.type }}</span></td><td class="spec">{{ r.material }}</td><td class="num">{{ r.qty }}{{ r.unit }}</td><td>{{ r.wh }}</td><td class="spec">{{ r.src }}</td><td class="spec">{{ r.date }}</td><td><span :class="['tag',r.sc]">{{ r.status }}</span></td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
<tr v-if="paged.length===0"><td colspan="9" class="empty">暂无数据</td></tr></tbody></table></div>
<PaginationBar :total="filtered.length" v-model="page" v-model:page-size="ps"/>
<FormModal :show="showForm" :title="editing?'编辑入库单':'新建入库单'" @close="showForm=false" @save="save"><div v-if="!editing" class="numbering-row"><label class="radio-label"><input type="radio" v-model="numberingMode" value="auto" /><span>自动编号</span></label><label class="radio-label"><input type="radio" v-model="numberingMode" value="manual" /><span>手动编号</span></label></div><div class="grid">
<div class="fg"><label>入库单号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'"/></div>
<div class="fg"><label>入库类型</label><select v-model="f.type"><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div>
<div class="fg"><label>物料 <span class="req">*</span></label><input v-model="f.material" placeholder="物料名称"/></div>
<div class="fg"><label>数量</label><input v-model.number="f.qty" type="number" min="1"/></div>
<div class="fg"><label>单位</label><select v-model="f.unit"><option>个</option><option>件</option><option>kg</option><option>张</option><option>根</option></select></div>
<div class="fg"><label>仓库</label><input v-model="f.wh" placeholder="如 原材料库"/></div>
<div class="fg"><label>来源单号</label><input v-model="f.src" placeholder="关联单号"/></div>
<div class="fg"><label>日期</label><input v-model="f.date" type="text" placeholder="2025-08-01"/></div>
<div class="fg"><label>状态</label><select v-model="f.status"><option>草稿</option><option>已入库</option></select></div>
<div class="fg full"><label>备注</label><textarea v-model="f.remark" rows="2"></textarea></div></div></FormModal>
<ConfirmDialog :show="showDel" title="确认删除" @confirm="doDel" @cancel="showDel=false"><p>确定删除入库单 <strong>{{ dt?.code }}</strong> 吗？</p></ConfirmDialog></div></template>
...
const showForm=ref(false);const editing=ref(false);const f=reactive({code:'',type:'采购入库',material:'',qty:1,unit:'个',wh:'',src:'',date:'',status:'草稿',remark:''});let ec=''
const numberingMode = ref('auto')
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;numberingMode.value='auto';f.code=`IN-${String(data.value.length+1).padStart(3,'0')}`;f.type='采购入库';f.material='';f.qty=1;f.unit='个';f.wh='';f.src='';f.date='';f.status='草稿';f.remark=''}showForm.value=true}
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;background:#e8f0fe;color:#1a73e8;}.tag.done{background:#e8f5e9;color:#2e7d32;}.tag.draft{background:#f5f5f5;color:#999;}

...
<div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索出库单号、物料..." class="search-input"/></div><select v-model="ft" class="filter-select"><option value="">全部类型</option><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建出库单</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">出库单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>类型</th><th>物料</th><th style="text-align:right;">数量</th><th>仓库</th><th>去向单号</th><th>日期</th><th>状态</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td><span class="tag">{{ r.type }}</span></td><td class="spec">{{ r.material }}</td><td class="num">{{ r.qty }}{{ r.unit }}</td><td>{{ r.wh }}</td><td class="spec">{{ r.dst }}</td><td class="spec">{{ r.date }}</td><td><span :class="['tag',r.sc]">{{ r.status }}</span></td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
...
<div class="fg"><label>出库单号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'"/></div>
<div class="fg"><label>出库类型</label><select v-model="f.type"><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div>
...
<div class="fg"><label>单位</label><select v-model="f.unit"><option>个</option><option>件</option><option>kg</option></select></div>
<div class="fg"><label>仓库</label><input v-model="f.wh"/></div>
<div class="fg"><label>去向单号</label><input v-model="f.dst"/></div>
<div class="fg"><label>日期</label><input v-model="f.date" type="text"/></div>
<div class="fg"><label>状态</label><select v-model="f.status"><option>草稿</option><option>已出库</option></select></div>

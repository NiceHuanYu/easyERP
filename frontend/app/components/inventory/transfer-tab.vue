...
<div class="erp-toolbar"><div class="erp-toolbar-left"><div class="erp-search-box"><span class="erp-search-icon">🔍</span><input v-model="s" placeholder="搜索调拨单号..." class="erp-search-input"/></div><select v-model="fs" class="erp-filter-select"><option value="">全部状态</option><option>草稿</option><option>已审核</option><option>已完成</option></select></div><div class="erp-toolbar-right"><button class="erp-btn erp-btn-primary" @click="openForm()">＋ 新建调拨</button></div></div>
...
<div class="erp-form-group"><label>状态</label><select v-model="f.status"><option>草稿</option><option>已审核</option><option>已完成</option></select></div>
...
<style scoped>.erp-search-input::placeholder{color:#bbb;}.erp-tag.done{background:#e8f5e9;color:#2e7d32;}.erp-tag.approved{background:#e3f2fd;color:#1565c0;}.erp-tag.draft{background:#f5f5f5;color:#999;}</style>
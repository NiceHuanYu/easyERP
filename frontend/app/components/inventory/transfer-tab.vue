...
<div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索调拨单号..." class="search-input"/></div><select v-model="fs" class="filter-select"><option value="">全部状态</option><option>草稿</option><option>已审核</option><option>已完成</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建调拨</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">调拨单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>物料</th><th style="text-align:right;">数量</th><th>源仓库</th><th>目标仓库</th><th>日期</th><th>状态</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td class="spec">{{ r.material }}</td><td class="num">{{ r.qty }}{{ r.unit }}</td><td>{{ r.from }}</td><td>{{ r.to }}</td><td class="spec">{{ r.date }}</td><td><span :class="['tag',r.sc]">{{ r.status }}</span></td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
<tr v-if="paged.length===0"><td colspan="8" class="empty">暂无数据</td></tr></tbody></table></div>
<PaginationBar :total="filtered.length" v-model="page" v-model:page-size="ps"/>
<FormModal :show="showForm" title="新建调拨" @close="showForm=false" @save="save"><div v-if="!editing" class="numbering-row"><label class="radio-label"><input type="radio" v-model="numberingMode" value="auto" /><span>自动编号</span></label><label class="radio-label"><input type="radio" v-model="numberingMode" value="manual" /><span>手动编号</span></label></div><div class="grid">
<div class="fg"><label>调拨单号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'"/></div>
<div class="fg"><label>物料 <span class="req">*</span></label><input v-model="f.material"/></div>
<div class="fg"><label>数量</label><input v-model.number="f.qty" type="number" min="1"/></div>
<div class="fg"><label>单位</label><select v-model="f.unit"><option>个</option><option>件</option><option>kg</option><option>张</option></select></div>
<div class="fg"><label>源仓库</label><input v-model="f.from" placeholder="调出仓库"/></div>
<div class="fg"><label>目标仓库</label><input v-model="f.to" placeholder="调入仓库"/></div>
<div class="fg"><label>日期</label><input v-model="f.date" type="text"/></div>
<div class="fg"><label>状态</label><select v-model="f.status"><option>草稿</option><option>已审核</option><option>已完成</option></select></div>
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.done{background:#e8f5e9;color:#2e7d32;}.tag.approved{background:#e3f2fd;color:#1565c0;}.tag.draft{background:#f5f5f5;color:#999;}

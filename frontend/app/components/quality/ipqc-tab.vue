<template><div class="tab-body"><div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索工单号、产品..." class="search-input"/></div><select v-model="fs" class="filter-select"><option value="">全部结果</option><option>合格</option><option>不合格</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建检验</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">检验单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>关联工单</th><th>产品</th><th>工序</th><th>检验类型</th><th>结果</th><th>检验员</th><th>日期</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td class="spec">{{ r.moCode }}</td><td>{{ r.product }}</td><td>{{ r.operation }}</td><td>{{ r.type }}</td><td><span :class="['tag',r.sc]">{{ r.result }}</span></td><td>{{ r.inspector }}</td><td class="spec">{{ r.date }}</td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
...
<div class="fg"><label>关联工单</label><input v-model="f.moCode"/></div>
<div class="fg"><label>产品</label><input v-model="f.product"/></div>
<div class="fg"><label>工序</label><input v-model="f.operation"/></div>
<div class="fg"><label>检验类型</label><select v-model="f.type"><option>首检</option><option>巡检</option><option>完工检</option></select></div>

<template><div class="tab-body"><div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索产品、订单..." class="search-input"/></div><select v-model="fs" class="filter-select"><option value="">全部结果</option><option>合格</option><option>不合格</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建OQC</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">检验单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>产品</th><th>销售订单</th><th style="text-align:right;">抽样数</th><th style="text-align:right;">不合格</th><th>结果</th><th>检验员</th><th>放行</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td>{{ r.product }}</td><td class="spec">{{ r.soCode }}</td><td class="num">{{ r.sample }}</td><td class="num">{{ r.defect }}</td><td><span :class="['tag',r.sc]">{{ r.result }}</span></td><td>{{ r.inspector }}</td><td>{{ r.release?'✅':'❌' }}</td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
...
<div class="fg"><label>检验单号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'"/></div>
<div class="fg"><label>产品</label><input v-model="f.product"/></div>
<div class="fg"><label>销售订单</label><input v-model="f.soCode"/></div>
<div class="fg"><label>抽样数</label><input v-model.number="f.sample" type="number" min="1"/></div>
<div class="fg"><label>不合格数</label><input v-model.number="f.defect" type="number" min="0"/></div>
<div class="fg"><label>检验结果</label><select v-model="f.result"><option>合格</option><option>不合格</option></select></div>
<div class="fg"><label>检验员</label><input v-model="f.inspector"/></div>
<div class="fg"><label>是否放行</label><select v-model="f.release"><option :value="true">放行</option><option :value="false">不放行</option></select></div>
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.pass{background:#e8f5e9;color:#2e7d32;}.tag.fail{background:#fce4ec;color:#c62828;}

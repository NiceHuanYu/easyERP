<template><div class="tab-body"><div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索检验单号、物料..." class="search-input"/></div><select v-model="fs" class="filter-select"><option value="">全部结果</option><option>合格</option><option>不合格</option><option>待检验</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建检验单</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">检验单号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>物料</th><th>供应商</th><th style="text-align:right;">抽样数</th><th style="text-align:right;">不合格数</th><th>检验员</th><th>结果</th><th>日期</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td class="spec">{{ r.material }}</td><td>{{ r.supplier }}</td><td class="num">{{ r.sample }}</td><td class="num">{{ r.defect }}</td><td>{{ r.inspector }}</td><td><span :class="['tag',r.sc]">{{ r.result }}</span></td><td class="spec">{{ r.date }}</td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
...
<div class="fg"><label>供应商</label><input v-model="f.supplier"/></div>
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.pass{background:#e8f5e9;color:#2e7d32;}.tag.fail{background:#fce4ec;color:#c62828;}.tag.pending{background:#fff3e0;color:#e65100;}

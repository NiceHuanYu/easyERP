<template><div class="tab-body"><div class="toolbar"><div class="toolbar-left"><div class="search-box"><span class="search-icon">🔍</span><input v-model="s" placeholder="搜索标准名称、物料..." class="search-input"/></div><select v-model="ft" class="filter-select"><option value="">全部类型</option><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div><div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建标准</button></div></div>
<div class="table-wrap"><table class="table"><thead><tr><th @click="sort('code')" class="sortable">标准编号{{ sf==='code'?(sa?'▲':'▼'):'' }}</th><th>标准名称</th><th>适用类型</th><th>检验项目</th><th>抽样方案</th><th>AQL</th><th>状态</th><th style="text-align:center;">操作</th></tr></thead>
<tbody><tr v-for="r in paged" :key="r.code"><td class="code">{{ r.code }}</td><td>{{ r.name }}</td><td><span class="tag">{{ r.type }}</span></td><td class="spec">{{ r.items }}</td><td>{{ r.sampling }}</td><td>{{ r.aql }}</td><td><span :class="['dot',r.status==='启用'?'on':'']"></span>{{ r.status }}</td><td class="acts"><button class="lnk" @click="openForm(r)">编辑</button><button class="lnk dgr" @click="del(r)">删除</button></td></tr>
...
<div class="fg"><label>标准编号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'"/></div>
<div class="fg"><label>标准名称 <span class="req">*</span></label><input v-model="f.name"/></div>
<div class="fg"><label>适用类型</label><select v-model="f.type"><option v-for="t in types" :key="t" :value="t">{{ t }}</option></select></div>
<div class="fg"><label>检验项目</label><input v-model="f.items" placeholder="如 尺寸/外观/硬度"/></div>
<div class="fg"><label>抽样方案</label><select v-model="f.sampling"><option>GB/T 2828.1</option><option>全检</option><option>自定义</option></select></div>
<div class="fg"><label>AQL值</label><input v-model="f.aql" placeholder="如 1.0"/></div>

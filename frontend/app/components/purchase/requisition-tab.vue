<template>
  <div class="tab-body">
    ...
    <FormModal :show="showForm" :title="editing?'编辑申请':'新建申请'" @close="showForm=false" @save="save">
      <div class="grid">
        <div class="fg"><label>申请单号</label><input v-model="f.code" :disabled="!editing" /></div>
...
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;f.code=`PR-${String(data.value.length+1).padStart(3,'0')}`;f.material='';f.qty=1;f.unit='个';f.dept='生产部';f.expectDate='';f.status='草稿';f.remark=''}showForm.value=true}

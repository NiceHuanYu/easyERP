  ...
  <div class="tab-body">
    <div class="toolbar">
      <div class="toolbar-left">
        <div class="search-box"><span class="search-icon">🔍</span><input v-model="s" type="text" placeholder="搜索订单号、供应商..." class="search-input" /></div>
    ...
    <FormModal :show="showForm" :title="editing?'编辑订单':'新建订单'" @close="showForm=false" @save="save">
      ...
      <div class="grid">
        <div class="fg"><label>订单号</label><input v-model="f.code" :disabled="!editing && numberingMode === 'auto'" /></div>
        ...
        <div class="fg"><label>交货日期</label><input v-model="f.delivery" type="text" placeholder="2025-08-20" /></div>
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.draft{background:#f5f5f5;color:#999;}.tag.pending{background:#fff3e0;color:#e65100;}
.tag.confirmed{background:#e3f2fd;color:#1565c0;}.tag.shipped{background:#e0f2f1;color:#00695c;}
.tag.arrived{background:#e8f5e9;color:#2e7d32;}.tag.completed{background:#f3e5f5;color:#6a1b9a;}.tag.cancelled{background:#fce4ec;color:#c62828;}

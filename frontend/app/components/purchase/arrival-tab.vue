...
const itemColumns = [
  ...
  { key: 'qty', label: '到货数量', type: 'number' as const, align: 'right' as const, min: 1, total: true },
  { key: 'unit', label: '单位', type: 'select' as const, options: ['个', '件', 'kg', '张'] },
]
...
function openForm(item?:Arrival){
  if(item){
    ...
    f.poCode='';f.supplier='';f.items=[];f.arrivalDate='';f.qcResult='待检验';f.status='待检验';f.remark=''
  }
  showForm.value=true
}

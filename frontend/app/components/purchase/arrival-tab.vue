  ...
  <div class="tab-body">
    <div class="toolbar">
      <div class="toolbar-left">
        <div class="search-box"><span class="search-icon">🔍</span><input v-model="s" type="text" placeholder="搜索到货单号、订单号..." class="search-input" /></div>
        <select v-model="fs" class="filter-select"><option value="">全部状态</option><option v-for="st in ss" :key="st" :value="st">{{ st }}</option></select>
      </div>
      <div class="toolbar-right"><button class="btn btn-primary" @click="openForm()">＋ 新建到货单</button></div>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="row in paged" :key="row.code">
            ...
            <td class="spec">{{ row.arrivalDate }}</td>
    ...
    <FormModal :show="showForm" :title="editing?'编辑到货单':'新建到货单'" @close="showForm=false" @save="save">
      <div class="grid">
        <div class="fg"><label>到货单号</label><input v-model="f.code" :disabled="!editing" /></div>
        ...
        <div class="fg"><label>到货数量</label><input v-model.number="f.qty" type="number" min="1" /></div>
        ...
        <div class="fg"><label>到货日期</label><input v-model="f.arrivalDate" type="text" placeholder="2025-07-25" /></div>
        <div class="fg"><label>检验结果</label><select v-model="f.qcResult"><option value="待检验">待检验</option><option value="合格">合格</option><option value="不合格">不合格</option></select></div>
...
function openForm(item?:any){if(item){editing.value=true;ec=item.code;Object.assign(f,{...item})}else{editing.value=false;f.code=`ARV-${String(data.value.length+1).padStart(3,'0')}`;f.poCode='';f.supplier='';f.material='';f.qty=1;f.unit='个';f.arrivalDate='';f.qcResult='待检验';f.status='待检验';f.remark=''}showForm.value=true}
...
.tag{display:inline-block;padding:2px 10px;border-radius:10px;font-size:11px;}.tag.pending{background:#fff3e0;color:#e65100;}.tag.pass{background:#e8f5e9;color:#2e7d32;}.tag.fail{background:#fce4ec;color:#c62828;}
.tag.waiting{background:#e3f2fd;color:#1565c0;}.tag.done{background:#f3e5f5;color:#6a1b9a;}

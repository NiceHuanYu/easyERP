...
<LazyQualityIqcTab v-if="activeTab==='iqc'" /><LazyQualityIpqcTab v-else-if="activeTab==='ipqc'" />
<LazyQualityOqcTab v-else-if="activeTab==='oqc'" /><LazyQualityNcTab v-else-if="activeTab==='nc'" />
<LazyQualityStdTab v-else-if="activeTab==='std'" /></div></div></template>
<script setup lang="ts">
definePageMeta({middleware:'auth'})
const route=useRoute()
const activeTab = computed(() => (route.query.tab as string) || 'iqc')
